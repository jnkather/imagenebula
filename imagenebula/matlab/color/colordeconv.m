function [iA, iS, iAS, iR, dA, dS, dAS, dR] = ...
	colordeconv(I, nstains, varargin)
%COLORDECONV color deconvolution.
% [IA, IS, IAS, DA, DS, DAS] = colordeconv(I, NSTAINS, VARARGIN)
%
...
function [Ds] = compute_optical_density(Is)
	npixels = size(Is, 1);
	nchannels = size(Is, 2);
    maxIs = max(Is, [], 1);
    logMaxIs = log(maxIs);
